...
| Sales Analysis | Total Sales by region (pie), by store (column), daily Total Sales trend (line), and four slicers (brand, category, customer segment, city) |
| Product Analysis | Total Sales by category, Total Profit by category, and a product table (`product_name` + `Total Sales`) |
| Customer Analysis | Customer Count by segment (donut, 1,085 Regular / 508 Occasional / 407 Premium), Total Sales by segment (bar), customer table (`customer_id` + `Total Sales`); the Repeat Customer % card is on the Executive Overview only |
...
**Not built / known rough edges:** Top-10/Bottom-10 Top-N filters, profit margin by product, Revenue by Channel and Orders by Status (blocked by the schema gap above), a Repeat-vs-New split, and any layout polish (several visuals are default-sized or overlapping), and the store chart merges stores that share a name (`store_name` is not unique; use `store_id`). The spec below is the target design; treat it as the remaining to-do list.
